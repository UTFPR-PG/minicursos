...
\subsection{Linux}
Após gravar a iso da distro desejada em um mídia como CD ou pendrive, pode-se usar o programa \emph{UNetbootin} para isso, deve-se inicializar o sistema a partir dessa isso. O mais comum seria colocar a iso no PC e este inicializar automaticamente a isso, caso isso não acontece é necessário trocar a ordem de boot.

\begin{itemize}
    \item Abra a BIOS ou UEFI\@. Normalmente as teclas \emph{f12}, \emph{esc} tem essa função. Caso nenhuma delas funciona procure a tecla para o seu modelo de PC.
    \item Vá atá a seção BOOT.
    \item Coloque em primeiro opção iniciar da mídia desejada, CD ou pendrive, caso a opção errada seja colocada a BIOS/UEFI não ira carregar o Linux.
    \item Reinicie.
\end{itemize}
