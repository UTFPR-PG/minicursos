...
Um Live CD é basicamente uma disto Linux instalada em um CD, desse modo é possível utilizar a distro Linux sem ter que instala-la. Apesar de algumas distros não anunciarem um Live CD a própria iso funciona como um Live CD, por exemplo as distro Ubuntu e Mint não anunciam um Live CD, mas ao colocar a iso em um CD pode-se testa-la sem instalar.

Um Live CD não precisa ser, estritamente, gravado em um CD, por exemplo um pendrive funcionará do mesmo modo, além de ser mais pratico. A principal desvantagem dese método é a velocidade baixa de execução, pois é necessário ficar constantemente lendo a mídia utilizada, que normalmente é mais lenta do que um HD.
...
\begin{itemize}
    ...
    \item Iniciar o computador através daquela mídia, pode ser necessário alterar a ordem de boot na bios (TODO Adicionar mais informações ?).
...
A instalação começa com o particionamento do disco rígido, particionar é dividir o espaço total em espaços menores, normalmente isto é feito para que cada espaço tenha uma utilização diferente. Exemplo: normalmente ao fazer \emph{Dual-Boot} existe uma partição onde fica o Windows e outra onde fica o Linux.

Após particionar o HD deve-se formatar a partição, o que basicamente significa apagar todos os dados que existem nela, e escolher um sistema de arquivo para a partição, um sistema de arquivo é quem decide como armazenar os dados naquele partição, existem vários sistemas de arquivo sugerimos o ext4 por ser o mais novo e estável.

Após isso a partição criada deve ser escolhida para como local de instalação, depois é so esperar a instalação acabar, pode ser necessário responder algumas perguntas para ajeitar o fuso horário, teclado, etc...
