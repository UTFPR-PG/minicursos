...
O Windows pode ser instalado em apenas uma partição, entretanto a instalação que será mostrada aqui precisa de duas partições para o Linux, uma pequena com o mesmo tamanho da ram, se tiver 6G de ram esta partição deve ter 6G, e outra maior onde será instalado o Linux.
...
A partição menor, aquela com o mesmo tamanho da ram, será usada para \emph{SWAP} que é a memória virtual, basicamente usa-se o HD como se fosse memória RAM.
...
Após gravar a iso da distro desejada em um mídia como CD ou pendrive, pode-se usar o programa \emph{UNetbootin} para isso, deve-se inicializar o sistema a partir dessa isso. O mais comum seria colocar a iso no PC e este inicializar automaticamente a iso, caso isso não acontece é necessário trocar a ordem de boot.

\begin{itemize}
    \item Abra a BIOS ou UEFI\@. Normalmente as teclas \emph{f12}, \emph{esc} tem essa função. Caso nenhuma delas funcione procure a tecla para o seu modelo.
    \item Vá atá a seção BOOT.
    \item Coloque em primeiro a opção iniciar da mídia desejada, CD ou pendrive, caso a opção errada seja colocada a BIOS/UEFI não ira carregar o Linux.
    \item Reinicie.
\end{itemize}
...
Na imagem~\ref{fig:elementary-partition} cria-se a tabela de partições para um disco.

\begin{figure}[!h]
  \centering
  \includegraphics[scale=.3]{imagens/elementary-install-04.png}
  \caption{Criação da tabela de partições.}
  \label{fig:elementary-partition}
\end{figure}

Na imagem~\ref{fig:elementary-root} a `/' raiz do sistema é criada, note que o ponto de montagem foi escolhido e o sistema de arquivos é EXT4\@.
...
Na imagem~\ref{fig:elementary-swap} será criada a partição de SWAP\@. Como regra para o tamanho dessa partição utilize a mesmo tamanho da ram disponível.
% O mesmo tamanho da RAM é utilizado para ser possível hibernar caso toda a RAM esteja cheia, referencia https://wiki.archlinux.org/index.php/Suspend_and_Hibernate.
...
Existem algumas configurações que devem ser feitas antes desse menu. A primeira é o horário, simplesmente clique no mapa perto de São Paulo. A segunda é sobre o layout do teclado, novamente selecione português do Brasil. A ultima será sobre o usuário como pode ser vista na imagem~\ref{fig:elementary-info}  
