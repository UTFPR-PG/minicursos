...
Um Live CD é uma disto Linux instalada em uma mídia externa como um CD ou pendrive, desse modo é possível utilizar a distro Linux sem ter que instala-la. Apesar de algumas distros não anunciarem possuir um Live CD a iso `normal' funciona como um Live CD, por exemplo as distro Ubuntu e Mint não anunciam um Live CD, mas ao colocar a iso em uma mídia pode-se testa-la sem a necessidade de instalar.

A principal desvantagem é que a execução é lenta, pois fica limitada a mesma velocidade de leitura da mídia utilizada, que acaba sendo menor do que a velocidade de leitura de um HD.
...
Na imagem~\ref{fig:ubuntu-live} é possível ver a distro Ubuntu mostrando uma opção para testar o Ubuntu sem instala-lo. Algumas distros permitem instalar mesmo após ter sido iniciado como um live CD.

% (TODO Adicionar mais informações sobre alterar a ordem de boot?).
\begin{itemize}
    \item Colocar a mídia com o live CD no computador.
    \item Iniciar o computador através daquela mídia, pode ser necessário alterar a ordem de boot na bios.
    \item Escolher a opção testar, se existir.
\end{itemize}

\section{Instalação}
Caso você nunca tenha instalado o Linux antes essa parte servirá de apoio durante a instalação. Aqui será discutido alguns conceitos necessarios para poder realizar uma instalação simples do Linux, seguida de um pequeno guia.

A primeira coisa a entender é o \emph{boot} 
...
O Windows pode ser instalado em apenas uma partição, entretanto a instalação que será mostrada aqui precisa de duas partições para o Linux, uma pequena 3G\~5G e outra maior onde será instalado o Linux.

Cada uma dessas partições tem um sistema de arquivo, que diz como cada informação deve ser guardada no disco, desse modo organizando-o. Sem um sistema de arquivos seria impossível dizer onde começa e termina cada `coisa' gravada no disco. O windows usa, normalmente, um sistema de arquivo chamado de NTFS enquanto o Linux, nessa instalação, usará o EXT4.

O Linux organiza seus arquivos e diretórios de uma forma diferente do Windows, isto é, não existe c:\textbackslash no Linux, apenas `\textbackslash' que é chamado de \emph{root} ou raiz. Todo o resto do sistema é instalado a partir da raiz. Depois da raiz nos temos algumas pastas importantes por exemplo \emph{/home} onde fica gravado todos os arquivos do usuário.

A partição menor, aquela de 3G\~5G, será usada para \emph{SWAP} que é a memória virtual, basicamente usa-se o HD como se fosse memória RAM.
...
\begin{itemize}
    ...
    \item Escolha o espaço para diminuir, sugiro no mínimo 25G.
...
\subsection{Linux}
O modo de instalar o Linux irá variar dependendo da distro usada, pois cada uma tem um instalador diferente. Alguns instaladores provem uma interface gráfica. Enquanto outros tem apenas uma linha de comando.

A instalação começa com o particionamento do disco rígido, particionar é dividir o espaço total em pedaços de espaço menor. Normalmente isto é feito para que cada espaço tenha uma utilização diferente. Exemplo: normalmente ao fazer \emph{Dual-Boot} existe uma partição para o Windows e outra para o Linux.

Após particionar o HD deve-se formatar a partição, o que significa apagar todos os dados que existem na partição, tome cuidado para não formatar a partição errada poise se isso acontecer todos os dados serão apagados. E escolher um sistema de arquivo para a partição, um sistema de arquivo é quem decide como armazenar os dados naquele partição, existem vários sistemas de arquivo sugerimos o ext4 por ser o mais novo e estável. 

Após isso a partição criada deve ser escolhida para como local de montagem da `/', a \emph{root} ou raiz é o ponto inicial do sistema, todo o resto é criado a partir da raiz. Pense nela como o c:\textbackslash do windows. A partição `/' deve ser grande o suficiente para acomodar a distro mais eventuais programas que serão instalados, escolha um tamanho de acordo com seu HD, note que o mínimo aconselhável ficaria em torno dos 10G.

Uma segunda partição deve ser criada, com tamanho médio de 5G, ao formatar esta partição escolha o ponto de montagem como \emph{SWAP}, que será o local que o Linux usará para memoria virtual, basicamente usar o HD como se fosse memória RAM, caso esta fique cheia.


TODO adicionar fotos da instalação.
