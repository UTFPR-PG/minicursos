...
Quando utilizamos um computador, estamos acostumados a usar a interface gráfica. Isso significa que fazemos bastante uso do mouse para termos uma garantia de intuitividade. A CLI (\emph{Command Line Interface}) não utiliza esses preceitos que estamos acostumados. Ela necessita de comandos para realizar tarefas como a criação de uma pasta, ou fazer a copia de um arquivo, entre outros. No começo pode parecer difícil decorar esses comandos, mas com o tempo e costume, a utilização da CLI fará com que a navegabilidade seja muito mais rápida.
...
A mudança de diretórios, ou seja, pastas, deve ser feita através do comando \textbf{cd} (``change directory''). Há alguns atalhos que podem ser utilizados na mudança de diretórios: o \textbf{$\sim$} é um atalho para o ``/home/usuario'', um \textbf{.} significa o diretório atual e \textbf{..} é equivalente ao diretório anterior. É importante lembrar que o / (``root'') também é um diretório!\\
...
\com{cd /temp/\\\$ rmdir ./NovaPasta/Pasta2}\\
...
\item\cominline{tar -cvf Arquivo.tar.gz caminho\_para\_pasta/} - Comprimir arquivos na pasta indicada pelo caminho.
\item\cominline{tar -xzf Arquivo.tar.gz} - Extrair arquivos de um .tar.gz.
\end{itemize}
